...
velocity <= 10  ->  penalty = 1.0 (정상, 패널티 없음)
10 < velocity <= 20  ->  penalty = 1.0 - 0.5 * (velocity - 10) / 10  (선형 감쇠)
velocity > 20  ->  penalty = 0.5 / (1 + log2(velocity / 20))  (로그 감쇠)
```

2단계 임계값 구조:

- 1단계 (10~20표/시간): 선형으로 점수가 서서히 감소. 10표/시간에서 패널티 없음 -> 20표/시간에서 50% 감소
- 2단계 (20표/시간 초과): 로그 스케일로 추가 감쇠. 이미 50% 깎인 상태에서 더 깎임

예시:

- 시간당 5표: penalty 1.0 (정상)
- 시간당 10표: penalty 1.0 (경계, 아직 정상)
- 시간당 15표: penalty 0.75 (점수 25% 감소)
- 시간당 20표: penalty 0.5 (점수 50% 감소)
- 시간당 40표: penalty 0.25 (점수 75% 감소)
- 시간당 80표: penalty 0.17 (점수 83% 감소)

특징:

- 2단계 임계값으로 경미한 홍보와 심한 어뷰징을 구분하여 대응
- 시간당 10표부터 서서히 감쇠가 시작되어 갑작스러운 점수 변화 방지
...
### Case 5: 좋아요 15개, 업로드 1시간 경과, 단톡방 홍보 의심

- Wilson Score: 중간
- Time Decay: 0.94 (매우 최근)
- Exploration: 낮음
- Velocity: 시간당 15표 -> penalty = 0.75 (1단계, 점수 25% 감소)
- 결과: 서서히 감쇠가 적용되어 상위권에서 밀림. 3시간 후 velocity가 5표/시간으로 떨어지면 패널티 해제

### Case 6: 좋아요 25개, 업로드 1시간 경과, 심한 어뷰징

- Wilson Score: 중간~높음
- Time Decay: 0.94 (매우 최근)
- Exploration: 낮음
- Velocity: 시간당 25표 -> penalty = 0.5 / (1 + log2(25/20)) = 약 0.37 (2단계, 점수 63% 감소)
- 결과: 2단계 패널티가 강하게 적용되어 하위권으로 하락. 시간이 지나 velocity가 정상화되면 복귀
